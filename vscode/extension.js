...
function activate(context) {
    ...
    statusBarItem.show();
    // --- Commands ---
    context.subscriptions.push(vscode.commands.registerCommand('openply.start', startInTerminal), vscode.commands.registerCommand('openply.chat', openChat), vscode.commands.registerCommand('openply.explain', explainSelected), vscode.commands.registerCommand('openply.refactor', refactorSelected), vscode.commands.registerCommand('openply.addTests', addTestsForFile), vscode.commands.registerCommand('openply.fixDiagnostics', fixDiagnostics), vscode.commands.registerCommand('openply.toggleMode', toggleMode), vscode.commands.registerCommand('openply.selectModel', selectModel), vscode.commands.registerCommand('openply.showOutput', () => outputChannel.show()), vscode.commands.registerCommand('openply.webview', openWebviewPanel));
    // --- Code Actions Provider ---
    context.subscriptions.push(vscode.languages.registerCodeActionsProvider({ scheme: 'file' }, new OpenPLYCodeActionProvider(), { providedCodeActionKinds: [vscode.CodeActionKind.QuickFix] }));
}
...
function openWebviewPanel() {
    const panel = vscode.window.createWebviewPanel('openply', 'openPly Chat', vscode.ViewColumn.Beside, {
        enableScripts: true,
    ...
    panel.webview.onDidReceiveMessage(async (message) => {
        if (message.type === 'chat') {
            try {
                const { stdout } = await execAsync(`openply --json "${message.prompt.replace(/"/g, '\\"')}"`, { cwd: vscode.workspace.workspaceFolders?.[0]?.uri.fsPath || process.cwd() });
...
// --- Code Actions Provider ---
class OpenPLYCodeActionProvider {
